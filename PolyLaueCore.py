...
#CellParameters=[2.87086,2.87086,2.87086,90,90,90] # Unit cell parameters: a, b, c, alpha, betta, gamma, Angstr., degrees
# Fe(bcc)

CellParameters=[2.463,2.463,3.948,90,90,120] # Unit cell parameters: a, b, c, alpha, betta, gamma, Angstr., degrees
# Fe(hcp)
...
def find(ang_tol=0.07, res_lim=0.4, ref_thr=3, ref_num=10):
    import math
...
def find_py(ang_tol=0.07, res_lim=0.4, ref_thr=3, ref_num=10):
    import math
    global EnergyHighest, CellParameters
    with np.load('geosetup.npz') as geo_arr:
        det_org=geo_arr['iitt1']
...
def track(ang_tol=0.07, ang_lim=29, res_lim=0.3, ref_thr=3):
    import math
...
def track_py(ang_tol=0.07, ang_lim=29, res_lim=0.3):
    import math
    global EnergyHighest
    with np.load('geosetup.npz') as geo_arr:
        det_org=geo_arr['iitt1']
